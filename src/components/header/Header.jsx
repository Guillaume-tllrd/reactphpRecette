        ...
        <>
            <header className="p-4">
                <div className='flex justify-between items-center'>
                    <Menu onClick={toggleMenuBurger} className="hover:cursor-pointer transition-transform transform hover:scale-110" />
                    ...
                    <div className='flex'>
                        {user && user.role === "admin" && <Link to="/dashboard"><Settings className='w-6 h-6 transition-transform transform hover:scale-110'/></Link>}
                    {user && user.id ? (<button className="w-6 h-6 transition-transform transform hover:scale-110" onClick={() => dispatch(getSessionLogOut())} ><LogOut/></button>) : (
                        <Link to='/login'>
                        <User className=" w-6 h-6 transition-transform transform hover:scale-110" />
                    </Link>
                    )}
                    </div>
                </div>
