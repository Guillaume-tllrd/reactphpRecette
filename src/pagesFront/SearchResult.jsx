...
const SearchResult = () => {
    ...
    return (
        ...
        <div className='bg-amber-100 py-5 pb-5'>
            ...
            {searchTagRecipe && <h1 className='font-scope text-2xl text-center mb-6'>"Explore our recipes with "{searchTagRecipe}" :</h1>}
            {searchTagArticle && <h1 className='font-scope text-2xl text-center mb-6'>"Explore our articles with "{searchTagArticle}" :</h1>}
            ...
            {(searchRecipe?.length > 0 || countryRecipe?.length > 0 || tagRecipe?.length > 0) ? (
                    ...
                    <div className='xl:w-[1350px] mx-auto'>
                        <ul className='flex flex-wrap gap-10 xl:gap-0'>
                            {searchTagArticle && tagArticle.map((article) => (
                                <ArticlePost article={article} key={article.id} />
                            ))}
                        </ul>
                    </div>
                ) : (
                    <p className='text-center'>No recipes or articles found</p>
                )}
        </div>
        <Footer />
    </div>
